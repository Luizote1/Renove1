...
function aplicarTema(nomeTema) {
    ...
    // Remove todos os temas e aplica o novo
    ...
    // Troca o logotipo conforme o tema
    if (logotipo) {
        const imgCaminho = nomeTema.includes('escuro') 
            ? "./Imgs/logotipo_dark.png" 
            : "./Imgs/logotipo.png";

        const imgCaminhoAlternativo = nomeTema.includes('escuro') 
            ? "../Imgs/logotipo_dark.png" 
            : "../Imgs/logotipo.png";

        // tenta primeiro o ./Imgs/
        logotipo.src = imgCaminho;

        // se der erro de carregamento, usa o ../Imgs/
        logotipo.onerror = () => {
            logotipo.src = imgCaminhoAlternativo;
        };
    }

    // Marca o tema ativo na lista
    listaTemasOpcoes.forEach(li => {
        li.classList.toggle('tema-ativo', li.dataset.tema === nomeTema);
    });
}
...
function atualizarNavegacao() {
    ...
    if (usuarioLogado === 'true') {
        ...
        navConta.innerHTML = '<a href="#" onclick="fazerLogout(); return false;">👤CONTA</a>';
